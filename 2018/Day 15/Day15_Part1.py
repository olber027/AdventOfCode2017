...
def getCharacterAt(x, y, characters):
    result = [c for c in characters if c.position[0] == x and c.position[1] == y]
    return result[0] if result else None

def findPath(start, dest, map):
    pass

def findClosestEnemy(position, enemyRace, map):
    pass

def isInRange(position, target, range=1):
    pass

class Character(object):
    def __init__(self, race, attackPower, hp, startPosition):
        self.race = race
    ...
    def act(self, map):
        enemy = findClosestEnemy(self.position, "goblin" if self.race == "elf" else "elf", map)
        if enemy is None:
            return False
        if isInRange(self.position, enemy):
            self.attack(enemy)
        else:
            self.move(enemy, map)
        return True
